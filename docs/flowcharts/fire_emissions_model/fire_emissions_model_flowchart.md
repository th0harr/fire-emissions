...
flowchart TD
    ...
    K --> L[Stage 1: build affected-stock components]
...
flowchart TD
    ...
    G --> G1[Direct = whole dwelling carbon stock]
    G --> G2[Replacement = whole dwelling embodied CO2]
```
...
flowchart TD
    ...
    B --> F[Calculate direct affected fractions]
    C --> G[Calculate replacement affected fractions]

    D --> H[Load origin room stock]
    E --> I[Load dwelling stock]
    H --> J[Load origin room embodied CO2]
    I --> K[Calculate dwelling embodied CO2]

    F --> L[Build direct carbon-stock components]
    G --> M[Build replacement embodied-CO2 components]

    L --> N[Stage 1 component rows]
    M --> N
```
...
flowchart TD
    ...
    B --> D[Direct affected carbon = whole dwelling carbon]
    C --> E[Replacement embodied CO2 = whole dwelling embodied CO2]

    D --> F[Use complete-combustion transition]
    F --> G[Calculate direct CO2 / CO]

    E --> H[Replacement embodied CO2 output]
```

Resolution:

```text
No area scaling is needed for the first-pass model.
